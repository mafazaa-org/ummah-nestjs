...
import {
  FriendRequest,
  FriendRequestStatus,
} from '../friend-requests/schemas/friend-request.schema';

async function bootstrap() {
  const app = await NestFactory.createApplicationContext(AppModule);

  const userModel = app.get<Model<User>>(getModelToken(User.name));
  const friendRequestModel = app.get<Model<FriendRequest>>(
    getModelToken(FriendRequest.name),
  );

  console.log('🔧 Starting friends data migration...');

  // Get all accepted friend requests
  const acceptedRequests = await friendRequestModel
    .find({
