...
async function deleteInvalidConversations() {
  ...
  try {
    ...
    for (const conversation of conversations) {
      ...
      if (conversation.type === 'private' && (conversation.participants.length < 2 || messages.length === 0)) {
        console.log(`❌ Invalid conversation ${conversationId}:`);
        console.log(`   Participants: ${conversation.participants.length} (should be 2)`);
    ...
    console.log(`   Valid conversations: ${conversations.length - deletedCount}`);
